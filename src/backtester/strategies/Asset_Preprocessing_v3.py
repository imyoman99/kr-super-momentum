...
CONFIG = {
    ...
    'END_DATE': '2025-08-31',       # 백테스트 종료일
    'INITIAL_CASH': 100_000_000,    # 초기 투자 자본금 (1억 원)
    'MAX_POSITIONS': 3,             # 동시 보유 최대 종목 수 (집중 투자 전략)
    'POS_WEIGHT': 0.33,             # 종목당 할당 비중 (자산의 약 33%)
    
    # [리스크 관리 및 수익 보존]
    'STOP_LOSS_PCT': 0.04,          # 하락 손절 라인 (-4%)
    'TRAIL_TRIGGER_PCT': 0.25,      # 트레일링 스탑 활성화 기준 수익률 (25% 수익 시 작동)
    'TRAIL_STOP_PCT': 0.01,         # 고점 대비 허용 하락폭 (고점 대비 -1% 시 익절)
    
    'BREAK_EVEN_TRIGGER': 0.15,     # 본전 보호 활성화 수익률 (15% 수익 시 손절가를 매수가 위로 상향)
    'BREAK_EVEN_BUFFER': 0.1,       # 본전 보호 시 최소 마진 (10%)

    # [매매 효율화]
    'TIME_STOP_DAYS': 14,           # 시간 제한 손절 (진입 후 14일간 보유)
