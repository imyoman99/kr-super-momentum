...
_suffix = ""
if CONFIG.get("USE_REGIME_FILTER") and not CONFIG.get("COMPARE_REGIMES"):
    suffix_val = str(CONFIG.get("REGIME_OUTPUT_SUFFIX", "")).strip()
    if suffix_val:
        _suffix = f"_{suffix_val}"

RESULT_PATH = BASE_DIR / "strategies" / f"Asset_List_Final{_suffix}.csv"

ANALYTICS_DIR = BASE_DIR.parent / "analytics"

# 분석 결과물(차트, 리포트)을 순차적으로 저장할 루트 폴더
RESULTS_BASE_DIR = BASE_DIR.parent.parent / "results"

sys.path.append(str(BASE_DIR.parent.parent))  # 프로젝트 루트(src) 추가
